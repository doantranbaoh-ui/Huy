...
# -*- coding: utf-8 -*-
"""
Telegram Bot tự động cày view YouTube - Tối ưu tốc độ 2 phút/view
- Tự động nhận file proxy.txt từ Telegram, merge không xóa
- Hỗ trợ SOCKS5 không giới hạn
- Chạy 24/7 với web server giữ tiến trình
- Tốc độ cày: 2 phút/view (tối ưu cho YouTube)
"""
...
# ===== CẤU HÌNH CÀY VIEW - TỐI ƯU 2 PHÚT/VIEW =====
YOUTUBE_WATCH_URL = "https://www.youtube.com/watch?v={video_id}"
MAX_WORKERS = 30  # Tăng số luồng để cày nhanh
VIEW_RETRY = 2
PROXY_CHECK_INTERVAL = 1800
PROXY_RELOAD_INTERVAL = 3600
HEARTBEAT_INTERVAL = 300

# Thời gian xem tối thiểu để tính view (2 phút = 120 giây)
MIN_WATCH_TIME = 120
MAX_WATCH_TIME = 180
...
def merge_new_proxies():
    ...
    log_message(f"Merge: thêm {added}, master={len(PROXY_MASTER)}, sống={len(PROXY_LIST)}, chết={len(DEAD_PROXIES)}")
    return added
...
def check_proxy_alive(proxy_str: str, timeout: int = 10) -> bool:
...
def fetch_video_page(video_id: str, proxy_str: str = None, retry: int = VIEW_RETRY):
    """
    Cày view với thời gian xem từ 120-180 giây (2-3 phút)
    Mô phỏng hành vi xem video thực tế
    """
    ...
    for attempt in range(retry):
        try:
            ...
            # Bước 3: Request cuối - xác nhận đã xem hết
            final_time = random.randint(watch_time - 10, watch_time + 10)
            session.get(
                url + "&t=" + str(final_time),
                headers=headers,
                proxies=proxies,
                timeout=5
            )
            
            # Bước 4: Request phụ trợ để tăng độ tin cậy
            ...
            if attempt == retry - 1:
                if proxy_str:
                    ...
                    log_message(f"Proxy chết khi cày: {proxy_str}")
            else:
...
def run_view_task(video_id: str, count: int):
    ...
    success = 0
    failed = 0
    total_time = count * 150  # ~2.5 phút/view
    ...
    estimated_time = int((count * 150) / MAX_WORKERS)
    return success, f"Success: {success}/{count} | Proxy alive: {alive_count} | Time: ~{estimated_time}s"
...
async def start_cmd(client, message: Message):
    ...
    await message.reply_text(
        f"🤖 Bot cày view YouTube 24/7 (2 phút/view)\n"
        f"📌 Lệnh:\n"
        f"/view <video_id> <số_lượng> - Cày view (1-500)\n"
        f"/reload - Tải lại proxy từ file (merge)\n"
        f"/proxy - Xem proxy sống\n"
        f"/dead - Xem proxy chết\n"
        f"/addproxy <proxy> - Thêm proxy (http/socks5)\n"
        f"/upload - Upload file proxy.txt (merge nhanh)\n"
        f"/stats - Thống kê\n"
...
async def view_cmd(client, message: Message):
    ...
    msg = await message.reply_text(f"⏳ Đang cày {count} view (2 phút/view)...")
...
async def proxy_cmd(client, message: Message):
    with proxy_lock:
        if PROXY_LIST:
            ...
            await message.reply_text(f"❌ Không có proxy sống. Upload file hoặc /addproxy")
...
async def addproxy_cmd(client, message: Message):
    ...
    if len(parts) < 2:
        await message.reply_text("❌ Dùng: /addproxy http://user:pass@ip:port hoặc socks5://user:pass@ip:port")
        return
...
async def upload_cmd(client, message: Message):
    await message.reply_text("📤 Gửi file proxy.txt (đính kèm) để merge siêu nhanh")
...
async def handle_document(client, message: Message):
    ...
    msg = await message.reply_text("⏳ Đang tải và merge proxy siêu nhanh...")
